...
def copyCampaigns(srcCampaignId,targetGroupId):
    ...
    #     print targetCampaignId,targetCreativeId
        ...
        print "new_paign_id:%s;targetCampaignId:%s" % (cur.insert_id,targetCampaignId) 
        #可以防止主键id变化
        targetCampaignId = int(cur.insert_id)
    ...
    try:
        ...
        if count>0:
            ...
            for r in results:
                ...
                creativeId=r[6]
                maxCreativeId+=1
                insertCreativessql="insert into Creatives(CampaignID,`Name`,BidPrice,AdType,AdMarkup,`Status`) values(%s,'%s',%s,'%s','%s',%s)" % (targetCampaignId,Name,Bidprice,AdTpye,AdMarkUp,status)
                print insertCreativessql
                cur.execute(insertCreativessql)
                #防止获取最大id后有记录插入最大id变化
                maxCreativeId = int(cur.insert_id)
                #insert into banner
